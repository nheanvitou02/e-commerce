<template>
  <div
    class="relative max-w-sm bg-white shadow-md cursor-pointer group overflow-x-hidden"
    v-for="item in data"
    :key="item.id"
  >
        ...
        <div class="mt-1">
          <div class="text-left space-y-1">
            <div class="bg-transparent overflow-hidden">
              <span class="text-xs font-semibold line-clamp-1 text-gray-500">
                {{ item.brand.name }}
              </span>
              <p
    ...
    <div class="absolute top-2 right-4 transition-all space-y-2">
      ...
      >
        <svg
          :class="
            item?.customer_id?.find((e) => e == user.id) ? ' fill-red-500 ' : ''
          "
          width="20"
          height="20"
          viewBox="0 0 24 24"
